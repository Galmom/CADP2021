...
procedure carga(var p:plantas);
	...
	procedure cant_paises(nombre_cien:string20; var cantPaises, max_paises:integer; var nombre_paises:string20);
		begin
			if (cantPaises > max_paises) then
				max_paises:=cantPaises;
				nombre_paises:=nombre_cien;
		end;	

	var
		especie:plantas;
		minimo, cant_tipo, cant_vida, max1, max2, max_paises, i, cantPaises:integer;
	...
	begin
		...
		carga(especie);
		while (especie.paises <> 'zzz') do begin{3 va 320... como se hace el corte de 320}
			...
			while (especie.paises <> 'zzz') and (actual = especie.tipo_planta) do begin
				...
				writeln('cambio de paises inicio');{-----------------------------------------}
				...
				writeln('cambio de paises fin');{-------------------------------------------}
				cantPaises:=cantPaises+1;
				menor_tipo(especie.tipo_planta, cantPaises, min_nombre, minimo);
			end;
			cant_paises(especie.nombre_cien, cant_tipo, max_paises, nombre_paises);
